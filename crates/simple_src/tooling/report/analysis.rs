...
/// Artifact root: `$CARGO_TARGET_TMPDIR/quality` when compiled as a test
/// (the variable is test-only), otherwise `<repo>/output/report/artifacts`
/// so the `report` example is fully standalone.
pub fn artifact_root() -> PathBuf {
    match option_env!("CARGO_TARGET_TMPDIR") {
        Some(dir) => Path::new(dir).join("quality"),
