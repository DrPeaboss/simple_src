...
pub fn generate_report() {
    ...
    for c in &thdn_cases {
        ...
        // CSV alongside the SVG so plots.py overlays work without the
        // baseline assertion tests having run first.
        write_csv(&dir.join(format!("{}.csv", c.name)), &c.db, c.bin_hz, 4);
    }
    ...
    // ---- sweep artifacts (plots.py spectrograms) ---------------------------
    {
...
<p class="note">Generated by <code>cargo run -p simple_src --example report</code>.
...
"##,
    ...
    // 7. reproduce
    {
        html.push_str(&section(
            ...
            "repro",
            "<ul>
<li>Generate the report: <code>cargo run -p simple_src --example report</code></li>
<li>Artifacts live in <code>output/report/artifacts/</code> (or <code>$CARGO_TARGET_TMPDIR/quality/</code> when the spectral tests produce them); a local copy is written to <code>output/report/index.html</code></li>
